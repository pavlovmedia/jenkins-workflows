def call(branchName, prefix="Java", pom="./pom.xml", doObr=true) {
  ...
  stage ("${prefix} build") {
    ...
    if (branchName.endsWith("-release")) {
      mvnTargets = "clean install javadoc:aggregate source:jar javadoc:jar install deploy -DaltDeploymentRepository=release.builder.dev.pavlovmedia.corp::default::http://release.builder.dev.pavlovmedia.corp/nexus/content/repositories/releases/ -Ddocker.repo='nexus.dev.pavlovmedia.corp:5000'"
    }
    // Run the build
